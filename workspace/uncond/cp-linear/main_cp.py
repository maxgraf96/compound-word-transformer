...
from dataset import midi2corpus, analyzer
from dataset.analyzer import proc_one
from dataset.representations.uncond.cp import corpus2events
from dataset.representations.uncond.cp.constants import BEAT_RESOLUTION
...
class TransformerModel(nn.Module):
    ...
    def froward_output_sampling(self, h, y_type):
        ...
        type_word_t = torch.from_numpy(
            np.array([cur_word_type])).cuda().unsqueeze(0)
    ...
    def print_word_cp(self, cp, classes):
    ...
    def inference_from_scratch(self, dictionary):
        ...
        with torch.no_grad():
            ...
            for step in range(init.shape[0]):
                self.print_word_cp(init[step, :], classes)
            ...
            while True:
                ...
                self.print_word_cp(next_arr, classes)
    ...
    def inference_bar_from_prompt(self, dictionary, prompt, n_bars=1):
        event2word, word2event = dictionary

        # Prepare prompt
        init = np.array(prompt)
        ...
        with torch.no_grad():
            ...
            for step in range(init.shape[0]):
                ...
                words = init[step, :][None, ...]

                # Remove EOS from init if there
                if word2event['type'][words[0][3]] == 'EOS':
                    continue
                else:
                    # Use the line below to store both prompt and generated in MIDI output
                    # final_res.append(words)
                    h, y_type, memory = self.forward_hidden(
                        input_, memory, is_training=False)

            # print('------ generate ------')
            while cnt_bar <= n_bars:
                # sample others
                next_arr = self.froward_output_sampling(h, y_type)
                final_res.append(next_arr[None, ...])
        ...
        # Append EOS if not there
        if word2event['type'][next_arr[3]] != 'EOS':
            final_res.append([[0, 0, 0, event2word['type']['EOS'], 0, 0, 0]])
...
def generate_bars_from_midi_prompt(net, event2word, word2event, dictionary, midi_path, n_bars=1):
    # gen
    start_time = time.time()

    # Convert MIDI to representation
    midi_output_path_analysed = 'rec_analysed.mid'
    # Melody extraction and chord recognition
    analyzer.proc_one(midi_path, midi_output_path_analysed)
    # Quantise duration, velocity, bpm and append EOS token
    output_path_quantised = 'rec_quantised.pkl'
    midi2corpus.proc_one(midi_output_path_analysed, output_path_quantised)
    # Convert quantised to CP representation
    output_path_cp = 'rec_cp.pkl'
    corpus2events.corpus2event_cp(output_path_quantised, output_path_cp)

    events_list = pickle.load(open(output_path_cp, 'rb'))
    class_keys = events_list[0].keys()
    ...
    for eidx, e in enumerate(events_list):
        ...
        for k in class_keys:
            try:
                words_tmp.append(event2word[k][e[k]])
            except Exception as ex:
                if k == 'velocity':
                    # Velocity not found, use default of 80
                    print(e[k], "not found, using default velocity of 80.")
                    default_velocity = 'Note_Velocity_80'
                    words_tmp.append(event2word[k][default_velocity])
                elif k == 'duration':
                    # Duration not found, use default of 120 ticks
                    print(e[k], "not found, using default duration of 120 ticks.")
                    default_velocity = 'Note_Duration_120'
                    words_tmp.append(event2word[k][default_velocity])
                else:
                    raise ex
    ...
    # save
    output_path_words = 'rec_words'
    path_outfile = os.path.join(output_path_words + '.npy')
    np.save(path_outfile, words)

    # Override gendir for testing
    path_gendir = '.'

    conversion_time = time.time() - start_time

    try:
        path_outfile = os.path.join(path_gendir, 'gen_from_recorded.mid')

        res = net.inference_bar_from_prompt(dictionary, words, n_bars)
        write_midi(res, path_outfile, word2event)

        song_time = time.time() - start_time
        word_len = len(res)
        print('Conversion time: %.2f' % conversion_time)
        print('Song time: %.2f' % song_time)
...
if __name__ == '__main__':
    ...
    # -- inference -- #
        ...
        generate_bars_from_midi_prompt(net, event2word, word2event, dictionary, 'mido_created.mid', 1)
